...
//    typedef std::unordered_map<unsigned long, std::shared_ptr<Vertex>> HashVertex;
    typedef std::map<unsigned long, std::shared_ptr<Vertex>> HashVertex;//Hash表，根据键值存储对应数据
    typedef std::unordered_map<unsigned long, std::shared_ptr<Edge>> HashEdge;//无序存储
    typedef std::unordered_multimap<unsigned long, std::shared_ptr<Edge>> HashVertexIdToEdge;
    //std::unordered_multimap是一种允许有重复键值的无序map
